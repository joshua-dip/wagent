...
export async function POST(request: NextRequest) {
  try {
    ...
    // S3에 업로드
    const s3Key = await uploadFileToS3(fileBuffer, file.name, file.type);
    ...
    const product = new Product({
      ...
      fileName: s3Key.split('/').pop(), // S3 키에서 파일명 추출
      originalFileName: file.name,
      fileSize: file.size,
      filePath: s3Key, // S3 키를 파일 경로로 저장
      downloadCount: 0,
      rating: 0,
      reviewCount: 0,
      isActive: true
    ...
    return NextResponse.json({ 
      message: "상품이 성공적으로 업로드되었습니다. (AWS S3)",
      product: {
        id: product._id,
        title: product.title,
        price: product.price,
